...
def XGBtree_stats(booster_list, X, y, cv):
    ...
    and trees in cross-validation, using pre-trained boosters.
    
    Parameters:
    - booster_list: List of trained boosters from each fold.
...
def predict_with_trees_cv(booster_list, X_predict):
    """
    Make predictions for a new dataset (X_predict) using all trees and all folds
    with pre-trained boosters. The predictions are aggregated to compute summary statistics.
...
if __name__ == "__main__":
    ...
    # Train models once and store boosters
    booster_list = []
    for train_idx, _ in cv.split(X_train):
        X_train_fold, y_train_fold = X_train.iloc[train_idx], y_train.iloc[train_idx]
        model.fit(X_train_fold, y_train_fold)
        booster_list.append(model.get_booster())

    # Step 1: Out-of-sample prediction statistics for data for which we have y
    oos_summary_stats = XGBtree_stats(booster_list, X_train, y_train, cv)
    ...
    predict_summary_stats = predict_with_trees_cv(booster_list, X_predict)
