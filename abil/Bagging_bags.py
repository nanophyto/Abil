...
def Bagging_stats(model_list, X, y, cv):
    """
    Compute out-of-sample predictions and summary statistics across all folds
    and bags in cross-validation using pre-trained BaggingRegressor models.
    
    Parameters:
    - model_list: List of trained BaggingRegressor models from each fold.
    ...
    - summary_stats: A DataFrame with mean, std, and confidence intervals
                     computed across all folds and bags for each sample.
    """
    ...
    for fold_idx, (_, test_idx) in enumerate(cv.split(X)):
        ...
        bagging_model = model_list[fold_idx]
        
        # Collect predictions for each bag in the current model
        fold_bag_predictions = []
        for bag in bagging_model.estimators_:
            bag_preds = bag.predict(X_test)
            fold_bag_predictions.append(bag_preds)
        
        fold_bag_predictions = np.array(fold_bag_predictions).T  # shape: (n_samples, n_bags)
        ...
        for idx, test_sample_idx in enumerate(test_idx):
            ...
            all_fold_predictions[test_sample_idx].append(fold_bag_predictions[idx, :])
    
    # Combine predictions for each sample across all folds and bags
    combined_predictions = {}
    for sample_idx, preds_list in all_fold_predictions.items():
        # Combine all predictions for the sample (across folds and bags)
...
def predict_with_bags_cv(model_list, X_predict):
    """
    Make predictions for a new dataset (X_predict) using all bags and all folds
    with pre-trained BaggingRegressor models. The predictions are aggregated to compute summary statistics.
    ...
    """
    all_fold_predictions = []  # Collect predictions across all folds and bags

    for bagging_model in model_list:
        # Collect predictions for each bag in the current model
        fold_bag_predictions = []
        for bag in bagging_model.estimators_:
            bag_preds = bag.predict(X_predict)
            fold_bag_predictions.append(bag_preds)
        
        # Store predictions for this fold across all bags
        all_fold_predictions.append(np.array(fold_bag_predictions).T)  # shape: (n_samples, n_bags)
    
    # Concatenate predictions across all folds
    all_fold_predictions = np.concatenate(all_fold_predictions, axis=1)  # shape: (n_samples, (n_folds * n_bags))
...
if __name__ == "__main__":
    ...
    model = BaggingRegressor(estimator=KNeighborsRegressor(), n_estimators=50, random_state=42)
    ...
    for train_idx, _ in cv.split(X_train):
        ...
        bagging_model = BaggingRegressor(estimator=KNeighborsRegressor(), n_estimators=50, random_state=42)
        bagging_model.fit(X_train_fold, y_train_fold)
        model_list.append(bagging_model)

    # Step 1: Out-of-sample prediction statistics for data for which we have y
    oos_summary_stats = Bagging_stats(model_list, X_train, y_train, cv)
    ...
    predict_summary_stats = predict_with_bags_cv(model_list, X_predict)
