#%%
# Merge monthly WOA18 Phosphate and subset to 0-200m
...
sample_ds = xr.open_dataset(base_path + "woa18_all_p01_01.nc", decode_times=False,
                            drop_variables={'climatology_bounds','crs','depth_bnds',
                                            'p_dd','p_gp','p_ma','p_mn','p_oa','p_sd','p_se',
...
for i in range(0, 12):
    ...
    ds = xr.open_dataset(base_path + "woa18_all_p" + files[i] + "_01.nc",
                         decode_times=False,drop_variables={'climatology_bounds','crs','depth_bnds',
                                                            'p_dd','p_gp','p_ma','p_mn','p_oa','p_sd','p_se',
                                                            'lat_bnds','lon_bnds','time'}
                         )
    dr_out = regridder1(ds['p_an'],skipna=True, na_thres=0.75)
    df = dr_out.to_dataframe(name="po4")
...
df.set_index(['time', 'depth', 'lat', 'lon'], inplace=True)
ds = df.to_xarray().to_netcdf("/home/mv23682/Documents/Abil/studies/wiseman2024/env_data_processing/regridded_data/po4.nc")
