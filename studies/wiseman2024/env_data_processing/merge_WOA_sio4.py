#%%
# Merge monthly WOA18 Silicate and subset to 0-200m
...
sample_ds = xr.open_dataset(base_path + "woa18_all_i01_01.nc", decode_times=False,
                            drop_variables={'climatology_bounds','crs','depth_bnds',
                                            'i_dd','i_gp','i_ma','i_mn','i_oa','i_sd','i_se',
...
for i in range(0, 12):
    ...
    ds = xr.open_dataset(base_path + "woa18_all_i" + files[i] + "_01.nc",
                         decode_times=False,drop_variables={'climatology_bounds','crs','depth_bnds',
                                                            'i_dd','i_gp','i_ma','i_mn','i_oa','i_sd','i_se',
                                                            'lat_bnds','lon_bnds','time'}
                         )
    dr_out = regridder1(ds['i_an'],skipna=True, na_thres=0.75)
    df = dr_out.to_dataframe(name="sio4")
...
df.set_index(['time', 'depth', 'lat', 'lon'], inplace=True)
ds = df.to_xarray().to_netcdf("/home/mv23682/Documents/Abil/studies/wiseman2024/env_data_processing/regridded_data/sio4.nc")
